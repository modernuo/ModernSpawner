...
| Skill | Stubbed | `skill:` definitions parse and register, but nothing calls `ModernSpawnerEvents.OnSkillUsed` yet, so they never fire; wired once the upstream ModernUO hook lands (**D3**) |
...
| **D3** | Skill trigger source | Upstream `SkillCheck` hook via ModernUO PR | ModernUO #2636 open; wiring PR follows |

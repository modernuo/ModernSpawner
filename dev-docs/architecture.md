...
`Spawner`, `ProximitySpawner`, `RegionSpawner` get `[SerializableField] List<SpawnerEntry> _entries`
(BaseSpawner v12 → v13 `MigrateFrom` hands the list down). `SpawnerEntry : ISpawnerEntry`. `SpawnerDto.Entries`
becomes `List<SpawnerEntryDto>`; `ApplyDto` calls `CreateEntry`. `SpawnerGump`, `SpawnerControllerGump`,
`EditSpawnerCommand` read `ISpawnerEntry`. Estimated size: ~25 files in UOContent, one save migration.
...
- **Semantics.** Two classes: *event* triggers (proximity, speech, kill, skill, external) and *gate*
  triggers (game/wall time windows). A spawner with `TriggerActivated` does not spawn on its timer until
  `_triggered` is true. Event triggers set `_triggered` and run one spawn cycle; `_triggered` clears when
  the spawner is full or on `ResetTrigger()`. Gate triggers set/clear `_triggered` on open/close. With no
  gate triggers and no `TriggerActivated`, event triggers simply force a cycle (today's behaviour).
- **Liveness.** Triggers are registered whenever `TriggerActivated` and the spawner is not deleted,
  independent of `Running`; `Running` only controls the timer. Registration moves to the deferred
  `[AfterDeserialization(false)]` hook.
- **Kill.** `[OnEvent(nameof(CreatureEvents.CreatureDeathEvent))]` handler routes `bc.Spawner as
  ModernSpawner` with `bc.LastKiller`; `RequireAllDead` evaluated as `Spawned.Count <= 1` at death time.
...
statement   := assignment | call | 'if' expr 'then' block ('else' block)? 'end'?
assignment  := path '=' expr                   // target.Hue = 0x8000 ; spawner.Notes = "x"
...
- `GetSpawnPosition(entry, spawned, map)`: if `entry.PositioningRule` is set, run the parsed rule (cached
  ...
  `base.GetSpawnPosition`. Remove `UseSmartPositioning`, `MaxZDelta`, `DefaultPositioner`, `_spawnArea`
  (use `SpawnBounds`), and the two duplicate `IsValidWater` copies.
...
- A `SpawnerTestFixture` boots the ModernUO test server (`TestServerInitializer`, `TestMapDefinitions`),
  places a `ModernSpawner` on the internal test map, and exposes `Tick()`.
...
`OnStarted/OnStopped` virtuals, `SkillUsedEvent`, sector-range movement subscription (deferred).
